...
static s_ctlconfig ctlconfig;

static void usage(void);
static void init_ctlconfig(void);
static void parse_ctlcommandline(int, char **);
...
init_ctlconfig(void)
{
    ...
    ctlconfig.socket = strdup(DEFAULT_SOCK);
    ctlconfig.command = WDCTL_UNDEF;
}
 ...
 */
void
parse_ctlcommandline(int argc, char **argv)
{
    extern int optind;
    int c;
    ...
    while (-1 != (c = getopt(argc, argv, "s:h"))) {
        ...
        case 's':
            if (optarg) {
                free(ctlconfig.socket);
                ctlconfig.socket = strdup(optarg);
            }
            break;
    ...
    if (strcmp(*(argv + optind), "status") == 0) {
        ctlconfig.command = WDCTL_STATUS;
    } else if (strcmp(*(argv + optind), "stop") == 0) {
        ctlconfig.command = WDCTL_STOP;
    } else if (strcmp(*(argv + optind), "reset") == 0) {
        ctlconfig.command = WDCTL_KILL;
        if ((argc - (optind + 1)) <= 0) {
            fprintf(stderr, "wdctl: Error: You must specify an IP " "or a Mac address to reset\n");
            usage();
            exit(1);
        }
        ctlconfig.param = strdup(*(argv + optind + 1));
    } else if (strcmp(*(argv + optind), "restart") == 0) {
        ctlconfig.command = WDCTL_RESTART;
    } else {
...
wdctl_status(void)
{
    ...
    sock = connect_to_server(ctlconfig.socket);
...
wdctl_reset(void)
{
    ...
    strncat(request, ctlconfig.param, (64 - strlen(request) - 1));
    strncat(request, "\r\n\r\n", (64 - strlen(request) - 1));
...
main(int argc, char **argv)
{
    ...
    /* Init configuration */
	init_ctlconfig();
	parse_ctlcommandline(argc, argv);

    switch (ctlconfig.command) {
    case WDCTL_STATUS:
        wdctl_status();
        break;
