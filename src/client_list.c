...
client_list_add(const char *ip, const char *mac, const char *token)
{
    ...
    if (!ip) {
        debug(LOG_DEBUG, "Added a new client to linked list without IP, only MAC %s Token: %s", mac, token);
    }else{
        curclient->ip = safe_strdup(ip);
        debug(LOG_INFO, "Added a new client to linked list: IP: %s Token: %s", ip, token);
    }
    if (!mac) {
        debug(LOG_DEBUG, "Added a new client to linked list without mac, only IP %s Token: %s", ip, token);

    }else{
        curclient->mac = safe_strdup(mac);
        debug(LOG_INFO, "Added a new client to linked list, MAC %s Token: %s", mac, token);
    }
   if (!token) {
       debug(LOG_DEBUG, "Added a new client to linked list without Token");
   }else{
